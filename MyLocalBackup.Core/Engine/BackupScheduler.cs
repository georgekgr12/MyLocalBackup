...
namespace MyLocalBackup.Core.Engine
{
    public class BackupScheduler
    {
        ...
        public bool IsCancelling => _backupCts?.IsCancellationRequested ?? false;
        ...
        public void CancelBackup()
        {
            ...
            _pauseEvent.Set(); // Wake up if paused so cancellation can proceed
        }
        ...
        public void Start()
        {
            ...
            var lastBackup = _db.GetLastSuccessfulRestorePoint();
            TimeSpan delay;
            ...
            {
                // First run ever. Wait for the interval.
        ...
        {
            if (!TimeSpan.TryParse(_config.DailyBackupTime, out var targetTime))
                targetTime = new TimeSpan(2, 0, 0); // fallback to 02:00
        ...
        private void ScheduleNextRun(TimeSpan delay)
        {
            lock (_timerLock)
            {
                ...
                _timer = new Timer(async _ => await RunScheduledBackup(), null, safeDelay, Timeout.InfiniteTimeSpan);
            }
        }
        ...
        public void Pause()
        {
            _pauseEvent.Reset(); // Block BEFORE setting flag — closes the race window
            _isPaused = true;
        }
