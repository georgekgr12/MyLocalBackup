...
# Strip PDB files; they are not needed in release MSI.
...
"@
$pkgContent = $pkgContent -replace '(?ms)^\s*<Feature Id="MainFeature".*?</Feature>', $featureBlock
...
# Hashes are printed for manual release verification. Updates are delivered by
# downloading the installer from GitHub Releases, not through the app.
...
# Step 5: Copy EXE to Desktop (manual installer)
...
Write-Host "EXE installer: $env:USERPROFILE\Desktop\MyLocalBackupSetup_v$Version.exe"
Write-Host "MSI package: $msiDir\MyLocalBackupSetup.msi"
Write-Host ""
Write-Host "EXE SHA256: $exeHash"
Write-Host "MSI SHA256: $msiHash"
Write-Host ""
Write-Host "Include hashes in GitHub release notes if desired:" -ForegroundColor Yellow
Write-Host "EXE SHA256: $exeHash" -ForegroundColor Cyan
Write-Host "MSI SHA256: $msiHash" -ForegroundColor Cyan
Write-Host ""
Write-Host "Upload the EXE installer to GitHub Releases for users to download manually." -ForegroundColor Yellow
